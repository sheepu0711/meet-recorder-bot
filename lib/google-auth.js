...
/**
 * Lightweight signed-in check that does NOT navigate the page (so it won't
 * disrupt a manual login in progress). Looks for Google session cookies.
 */
async function hasSessionCookie(page) {
  try {
    ...
    const names = ['SID', '__Secure-1PSID', '__Secure-3PSID', 'SAPISID'];
    return cookies.some((c) => names.includes(c.name) && c.value && c.value.length > 5);
  } catch {
    return false;
  }
}
...
module.exports = { isSignedIn, hasSessionCookie, googleLogin, ensureLoggedIn };
